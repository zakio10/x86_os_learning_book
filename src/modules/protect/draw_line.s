draw_line:
        ...
        push    dword 0                     ;   -16| inc_x = 0; //X座標増分(基準軸:1 or -1)
        push    dword 0                     ;   -20| y0 = 0; //Y座標
        push    dword 0                     ;   -24| dy = 0; //Y増分
        push    dword 0                     ;   -28| inc_y = 0; //Y座標増分(基準軸:1 or -1)
...
.22E:                                           ;}
        ...
        ;---------------------------------------
        ;繰り返し回数(基準軸のドット数)(基準軸の長さが0な場合,繰り返し回数を1に設定し,1ピクセルの点を描画)
        ;---------------------------------------
...
%endif
        ...
        ;基準軸を更新
        ;---------------------------------------
        mov     eax, [esi - 8]                  ;   EAX = 基準軸増加(1 or -1);
        add     [esi - 0], eax                  ;
        ...
        add     eax, [edi - 4]                  ;   EAX += dy;  //増加(相対軸の描画幅)

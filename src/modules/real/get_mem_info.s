...
.10L:                                           ;do
        ...
        mov     di, .b0                         ;       ES:DI = バッファ(保存先)
        int     0x15                            ;       CF = BIOS(0x15, 0xE820); //ES:DIにメモリ情報保存される
...
put_mem_info:
        ...
        ;---------------------------------------
        ;レコード情報のデータタイプを文字列に変換
        ;---------------------------------------
        mov     bx, [si +16]                    ;//タイプを文字列で表示
        and     bx, 0x07                        ;BX = Type(0~5)
        shl     bx, 1                           ;BX *= 2; //要素サイズに変換(Double Word)
        add     bx, .t0                         ;BX += .t0; //テーブルの先頭アドレスを加算(オフセット)
